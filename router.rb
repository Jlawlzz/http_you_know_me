...
module Router
  ...
  def route_verb(request_array)
    if request_array[0] == "GET"
      route_get(request_array)
    else
      route_post(request_array)
    end
  end

  def route_get(request_array)

    if request_array[1] == '/hello'
      default_functions.hello_path
    elsif request_array[1] == '/datetime'
      default_functions.datetime_path
    elsif request_array[1] == '/shutdown'
      default_functions.shutdown_path
